...
export default{
  ...
  methods:{
    ...
    // 打开新增地址弹框
    openAddressModal(){
      this.userAction = 0;//设置用户行为类型
      this.checkedItem = {};//将checkItem设置为空，因为这个是model绑定的，如果里面有值会将值带进去
      this.showEditModal = true;//将弹窗打开
    },
    // 打开修改地址弹框
    editAddressModal(item){
      this.userAction = 1;
      this.checkedItem = item;//将item里的值，也就是选中的要修改的地址的值传入弹框里
      this.showEditModal = true;
    },
    delAddress(item){
      this.checkedItem = item;//最主要是拿他的id = =
      this.userAction = 2;
      this.showDelModal = true;
    },
    // 地址删除、编辑、新增功能 0 新增 1编辑 2删除
    submitAddress(){
    ...
    getCartList(){
      this.axios.get('/carts').then((res)=>{
        let list = res.data.cartProductVoList;//获取购物车中所有商品数据
        this.cartTotalPrice = res.data.cartTotalPrice;//商品总金额
